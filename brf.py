...
print('baseline: {}'.format(
    y_train.sum() * 1.0 /
    len(y_train)))  # the minority class makes up 1.7% of the training set
print('')
...
# class_weight = [p,n] would be the same as 'auto'. relative to 'auto', the
#   models below range from underweighting to overweighting the minority
...
for p in params:
    ...
    print('precision at {}: {} +/- {}'.format(
        K, np.mean(precision[:, K - 1]), np.std(precision[:, K - 1])))
